...
def generate_data(seed_):
    ...
    W = ( - U ** 2) + 1 * np.random.uniform( size = (data_size))
    Z1 = np.random.normal(size = (data_size)) * 0.1 - 1
    Z2 = np.random.normal(size = (data_size)) * 0.1 + 1
    Z = Lambda((1 - U) * Z1 + U * Z2) + 100 * np.random.uniform( size = (data_size))
    A = np.random.uniform(0, size = (data_size)) + 0.25 * np.sqrt(np.abs(U)) - 0.2 * Z
    Y = 3 * W - 0.1 * A - np.cos(A / 2 + 5 * U)
    ...
    for a_ in do_A:
        ...
        W_ = ( - U_ ** 2) + 1 * np.random.uniform( size = (100000))
        EY_do_A.append(np.mean( 3 * W_ - 0.1 * a_ - np.cos(a_ / 2 + 5 * U_)))
...
for seed_ in seed_list:
    for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
        for label_variance_in_lambda_opt_ in label_variance_in_lambda_opt_list:
            ...
            df_results.to_pickle("../../Results/SyntheticLowDim_ATE_Comparison_Misspecified_Setting3.1.pkl")
