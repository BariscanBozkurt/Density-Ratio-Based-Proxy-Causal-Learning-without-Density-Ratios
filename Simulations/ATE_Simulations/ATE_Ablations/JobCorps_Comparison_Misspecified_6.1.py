...
from utils.kernel_utils import Kernel, RBF, ColumnwiseRBF
...
class CustomZKernel(Kernel):
    def __init__(self, RBF1, RBF2):
        self.RBF1 = RBF1
        self.RBF2 = RBF2
    ...
    def __call__(self, X, Y = None):
        ...
        K1 = self.RBF1(jnp.hstack((X[:, 0:20], X[:, 40:])), jnp.hstack((Y[:, 0:20], Y[:, 40:])))
        K2 = self.RBF2(X[:, 20:40], Y[:, 20:40])
        self.RBF1.use_length_scale_heuristic = False
        self.RBF2.use_length_scale_heuristic = False
        K = K1 * K2
        return K
...
for seed_ in seed_list:
    for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
        for label_variance_in_lambda_opt_ in label_variance_in_lambda_opt_list:
            ...
            W = U  + jnp.array(np.random.normal(*U.shape)) / 1.
            Z = Lambda(U / (U.max(1) + 1e-3).reshape(-1, 1)) + (2 * np.random.uniform(*U.shape) - 1) 
            Z[:, 0:20] = np.random.normal(size = Z[:, 0:20].shape)
            Z[:, 40:] = np.random.normal(size = Z[:, 40:].shape)
            ...
            RBF_Kernel_Z1 = RBF(use_length_scale_heuristic = True, use_jit_call = True)
            RBF_Kernel_Z2 = RBF(use_length_scale_heuristic = True, use_jit_call = True)
            RBF_Kernel_Z = CustomZKernel(RBF_Kernel_Z1, RBF_Kernel_Z2)
            ...
            df_results.to_pickle("../../Results/JobCorps_ATE_Estimation_Comparison_Misspecified_Setting6.1.pkl")
