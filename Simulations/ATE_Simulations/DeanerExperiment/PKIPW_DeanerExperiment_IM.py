...
for data_seed_ in data_seed_list:
    for seed_ in seed_list:
        ...
        return_test = True
        W, Z, A, Y, W_test, Z_test, A_test, Y_test, do_A, EY_do_A = read_deaner_dataset(data_path, id_, data_seed_, return_test = return_test)
        ...
        A_transformed = A_transformed.reshape(data_size, -1)
        Z_transformed = Z_transformed.reshape(data_size, -1)
        W_transformed = W_transformed.reshape(data_size, -1)
        Y_transformed = Y_transformed.reshape(data_size, -1)

        A_test_transformed = A_transformer.transform(A_test)
        Z_test_transformed = Z_transformer.transform(Z_test)
        W_test_transformed = W_transformer.transform(W_test)
        ...
        test_dataset = PVTestDataSet(   treatment = A_test_transformed,
        ...
        df_results.to_pickle("../../Results/PKIPW_Deaner_IM.pkl")