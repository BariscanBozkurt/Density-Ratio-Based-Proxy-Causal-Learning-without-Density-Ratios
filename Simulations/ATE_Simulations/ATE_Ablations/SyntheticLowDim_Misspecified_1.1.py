...
def generate_data(seed_):
    ...
    W = Lambda(U) + 1 * np.random.uniform( size = (data_size))
    Z1 = np.random.normal(size = (data_size)) * 0.1 - 1
    Z2 = np.random.normal(size = (data_size)) * 0.1 + 1
    Z = ((1 - U) * Z1 + U * Z2) + 100 * np.random.uniform( size = (data_size))
...
for seed_ in seed_list:
    for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
        for label_variance_in_lambda_opt_ in label_variance_in_lambda_opt_list:
            ...
            df_results.to_pickle("../../Results/SyntheticLowDim_ATE_Comparison_Misspecified_Setting1.1.pkl")
