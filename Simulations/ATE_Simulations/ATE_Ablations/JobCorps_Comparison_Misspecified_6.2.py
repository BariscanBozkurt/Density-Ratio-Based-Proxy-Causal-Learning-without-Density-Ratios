...
for seed_ in seed_list:
    for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
        for label_variance_in_lambda_opt_ in label_variance_in_lambda_opt_list:
            ...
            W = Lambda(U / (U.max(1) + 1e-3).reshape(-1, 1) ) + (2 * np.random.uniform(*U.shape) - 1) * 1. 
            W[:, 0:20] = np.random.normal(size = W[:, 0:20].shape)
            W[:, 40:] = np.random.normal(size = W[:, 40:].shape)
            ...
            RBF_Kernel_W1 = RBF(use_length_scale_heuristic = True, use_jit_call = True)
            RBF_Kernel_W2 = RBF(use_length_scale_heuristic = True, use_jit_call = True)
            RBF_Kernel_W = CustomWKernel(RBF_Kernel_W1, RBF_Kernel_W2)
            ...
            df_results.to_pickle("../../Results/JobCorps_ATE_Estimation_Comparison_Misspecified_Setting6.2.pkl")
