...
kernelZ_length_scale_list = [0.5, 1.0, 1.5, 2.0, 'median_heur']

df_results = pd.DataFrame(columns = ["Algorithm", "Data_Size", "Seed", "Causal_MSE", "Causal_MAE", "eta_parameter", "sigma_square", "scale_data", "stage1_perc", "kernelZ_length_scale", "Algo_Run_Time"])

for n_plus_m in data_size_list:
    for seed_ in seed_list:
        for optimize_eta in optimize_eta_list:
            for scale_data in scale_data_list:
                for kernelZ_length_scale_ in kernelZ_length_scale_list:
                    for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
                        ...
                        if kernelZ_length_scale_ == 'median_heur':
                            RBF_Kernel_Z = RBF(use_length_scale_heuristic = True, use_jit_call = True)
                        else:
                            RBF_Kernel_Z = RBF(use_length_scale_heuristic = False, length_scale = kernelZ_length_scale_,  use_jit_call = True)
                        ...
                        Kernel_Alternative_Proxy_Dict = {
                            ...
                            "stage1_perc": stage1_perc,
                            "kernelZ_length_scale": kernelZ_length_scale_,
                            "Algo_Run_Time" : algo_run_time
                        }

                        df_results = pd.concat([df_results, pd.DataFrame([Kernel_Alternative_Proxy_Dict])], ignore_index=True)

                        df_results.to_pickle("../Results/Kernel_Alternative_Proxy_Synthetic_Experiment1_Kernel_Z_Bandwidth_AblationStudy.pkl")