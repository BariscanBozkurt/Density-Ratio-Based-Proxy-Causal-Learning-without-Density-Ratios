...
def generate_data(seed_):
    ...
    Z = ( - U ** 2) + 1 * np.random.uniform( size = (data_size))
    ...
    for a_ in do_A:
        ...
        W1_ = np.random.normal(size = (100000)) * 0.1 - 1
        W2_ = np.random.normal(size = (100000)) * 0.1 + 1
        W_ = Lambda((1 - U_) * W1_ + U_ * W2_ + 100 * np.random.uniform( size = (100000)))
        EY_do_A.append(np.mean( 3 * W_ - 2 * a_ - np.cos(a_ * 10 + 5 * U_)))
...
for seed_ in seed_list:
    for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
        for label_variance_in_lambda_opt_ in label_variance_in_lambda_opt_list:
            ...
            df_results.to_pickle("../../Results/SyntheticLowDim_ATE_Comparison_Misspecified_Setting3.2.pkl")
