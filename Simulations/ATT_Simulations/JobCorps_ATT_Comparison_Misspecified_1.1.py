...
for seed_ in seed_list:
    for a_prime in a_prime_list:
        ...
        for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
            ...
            W = U + (2 * jnp.array(np.random.uniform(*U.shape)) - 1) * 1.
            Z = Lambda(U / (U.max(1) + 1e-3).reshape(-1, 1) ) + jnp.array(np.random.normal(*U.shape)) / 1.
            Z = Z[:, :20]
            ...
            df_results.to_pickle("../Results/JobCorps_ATT_Estimation_Comparison_Misspecified_Setting1.1.pkl")
