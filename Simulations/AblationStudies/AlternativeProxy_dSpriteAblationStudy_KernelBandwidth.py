...
kernel_length_scale_heuristic_quantile_list = [0.1, 0.5, 0.9, 0.95, 0.99]
df_results = pd.DataFrame(columns = ["Algorithm", "Data_Size", "Seed", 
                                     "Causal_MSE", "Causal_MAE", "eta_parameter", 
                                     "sigma_square", "scale_data", "kernel", "stage1_perc",
                                     "length_scale_quantile", "validation_loss", "Algo_Run_Time"])
                        ...
                        for label_variance_in_eta_opt_ in label_variance_in_eta_opt_list:
                            for length_scale_quantile in kernel_length_scale_heuristic_quantile_list:
                                ...
                                Kernel_Alternative_Proxy_Dict = {
                                    ...
                                    "validation_loss" : model.final_second_stage_validation_loss.item(),
                                    "Algo_Run_Time" : algo_run_time
                                }

                                df_results = pd.concat([df_results, pd.DataFrame([Kernel_Alternative_Proxy_Dict])], ignore_index=True)

                                df_results.to_pickle("../Results/Kernel_Alternative_Proxy_dSpriteV3_KernelBandwidthAblation.pkl")